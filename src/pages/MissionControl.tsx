  ...
  const load = useCallback(async () => {
    ...
    try {
      await Promise.allSettled([
        ...
        bridgeQueryKey("tasks_pos_status").then(r => setData(p => ({ ...p, "tasks_pos_status": r }))).catch(() => setData(p => ({ ...p, "tasks_pos_status": [] }))),
      ]);
  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center justify-between flex-wrap gap-3">
        <div><h1 className="text-2xl font-bold">Mission Control</h1>
      ...
      <div className="space-y-4">
        ...
        <Sec title="Critical / Overdue" n={(data["tasks_cc_critical"] || []).length}>
        ...
        <Sec title="Status Breakdown" n={(data["tasks_pos_status"] || []).length}>
          <DT rows={data["tasks_pos_status"]} ld={ld}
            head={() => <tr className="border-b border-slate-700 bg-slate-900/50"><th className="px-3 py-2 text-left text-xs font-semibold text-slate-400 uppercase tracking-wider whitespace-nowrap">status</th><th className="px-3 py-2 text-left text-xs font-semibold text-slate-400 uppercase tracking-wider whitespace-nowrap">cnt</th></tr>}
