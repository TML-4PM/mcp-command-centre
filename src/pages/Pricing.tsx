  ...
  const load = useCallback(async () => {
    ...
    try {
      await Promise.allSettled([
        bridgeQueryKey("pricing_full_list").then(r => setData(p => ({ ...p, "pricing_full_list": r }))).catch(() => setData(p => ({ ...p, "pricing_full_list": [] }))),
        bridgeQueryKey("pricing_cat_ranges").then(r => setData(p => ({ ...p, "pricing_cat_ranges": r }))).catch(() => setData(p => ({ ...p, "pricing_cat_ranges": [] }))),
      ]);
  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center justify-between flex-wrap gap-3">
        <div><h1 className="text-2xl font-bold">Pricing</h1>
        <p className="text-slate-500 text-xs mt-0.5 font-mono">page_id:pricing · live · no hardcoded data</p></div>
