  ...
  const load = useCallback(async () => {
    ...
    try {
      await Promise.allSettled([
        bridgeQueryKey("infra_lambda_count").then(r => setKpis(p => ({ ...p, "infra_lambda_count": r[0]?.value ?? r[0] ?? "—" }))).catch(() => setKpis(p => ({ ...p, "infra_lambda_count": "err" }))),
        bridgeQueryKey("infra_s3_count").then(r => setKpis(p => ({ ...p, "infra_s3_count": r[0]?.value ?? r[0] ?? "—" }))).catch(() => setKpis(p => ({ ...p, "infra_s3_count": "err" }))),
        bridgeQueryKey("infra_tools_count").then(r => setKpis(p => ({ ...p, "infra_tools_count": r[0]?.value ?? r[0] ?? "—" }))).catch(() => setKpis(p => ({ ...p, "infra_tools_count": "err" }))),
        bridgeQueryKey("infra_build_total").then(r => setKpis(p => ({ ...p, "infra_build_total": r[0]?.value ?? r[0] ?? "—" }))).catch(() => setKpis(p => ({ ...p, "infra_build_total": "err" }))),
        bridgeQueryKey("infra_lambda_list").then(r => setData(p => ({ ...p, "infra_lambda_list": r }))).catch(() => setData(p => ({ ...p, "infra_lambda_list": [] }))),
        bridgeQueryKey("infra_lambda_by_category").then(r => setData(p => ({ ...p, "infra_lambda_by_category": r }))).catch(() => setData(p => ({ ...p, "infra_lambda_by_category": [] }))),
        bridgeQueryKey("infra_s3_list").then(r => setData(p => ({ ...p, "infra_s3_list": r }))).catch(() => setData(p => ({ ...p, "infra_s3_list": [] }))),
        bridgeQueryKey("infra_queue_dash").then(r => setData(p => ({ ...p, "infra_queue_dash": r }))).catch(() => setData(p => ({ ...p, "infra_queue_dash": [] }))),
        bridgeQueryKey("infra_tools_list").then(r => setData(p => ({ ...p, "infra_tools_list": r }))).catch(() => setData(p => ({ ...p, "infra_tools_list": [] }))),
        bridgeQueryKey("infra_alerts_list").then(r => setData(p => ({ ...p, "infra_alerts_list": r }))).catch(() => setData(p => ({ ...p, "infra_alerts_list": [] }))),
      ]);
  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center justify-between flex-wrap gap-3">
        <div><h1 className="text-2xl font-bold">Infra</h1>
        <p className="text-slate-500 text-xs mt-0.5 font-mono">page_id:infra · live · no hardcoded data</p></div>
      ...
      {Object.keys(kpis).length > 0 && (
        <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
          <SC label="Lambdas" v={kpis["infra_lambda_count"]} ld={ld} />
          <SC label="S3 Buckets" v={kpis["infra_s3_count"]} ld={ld} />
          <SC label="Tools" v={kpis["infra_tools_count"]} ld={ld} />
          <SC label="Build Actions" v={kpis["infra_build_total"]} ld={ld} />
        </div>
      )}
      <div className="space-y-4">
        <Sec title="Lambda Registry" n={(data["infra_lambda_list"] || []).length}>
          <DT rows={data["infra_lambda_list"]} ld={ld}
            ...
            row={(r, i) => <tr key={i} className="border-b border-slate-700/40 hover:bg-slate-700/20"><td className="px-3 py-2 text-xs text-slate-300 max-w-xs truncate">{String(r["function_name"] ?? "—")}</td><td className="px-3 py-2 text-xs text-slate-300 max-w-xs truncate">{String(r["category"] ?? "—")}</td><td className="px-3 py-2 text-xs text-slate-300 max-w-xs truncate">{String(r["runtime"] ?? "—")}</td><td className="px-3 py-2 text-xs text-slate-300 max-w-xs truncate">{String(r["memory_mb"] ?? "—")}</td><td className="px-3 py-2 text-xs text-slate-300 max-w-xs truncate">{String(r["status"] ?? "—")}</td></tr>}
          />
        </Sec>
        <Sec title="By Category" n={(data["infra_lambda_by_category"] || []).length}>
          <DT rows={data["infra_lambda_by_category"]} ld={ld}
            ...
            row={(r, i) => <tr key={i} className="border-b border-slate-700/40 hover:bg-slate-700/20"><td className="px-3 py-2 text-xs text-slate-300 max-w-xs truncate">{String(r["category"] ?? "—")}</td><td className="px-3 py-2 text-xs text-slate-300 max-w-xs truncate">{String(r["count"] ?? "—")}</td><td className="px-3 py-2 text-xs text-slate-300 max-w-xs truncate">{String(r["runtimes"] ?? "—")}</td></tr>}
          />
        </Sec>
        <Sec title="S3 Buckets" n={(data["infra_s3_list"] || []).length}>
          <DT rows={data["infra_s3_list"]} ld={ld}
            ...
            row={(r, i) => <tr key={i} className="border-b border-slate-700/40 hover:bg-slate-700/20"><td className="px-3 py-2 text-xs text-slate-300 max-w-xs truncate">{String(r["bucket_name"] ?? "—")}</td><td className="px-3 py-2 text-xs text-slate-300 max-w-xs truncate">{String(r["region"] ?? "—")}</td><td className="px-3 py-2 text-xs text-slate-300 max-w-xs truncate">{String(r["total_objects"] ?? "—")}</td><td className="px-3 py-2 text-xs text-slate-300 max-w-xs truncate">{String(r["primary_use"] ?? "—")}</td><td className="px-3 py-2 text-xs text-slate-300 max-w-xs truncate">{String(r["status"] ?? "—")}</td></tr>}
          />
        </Sec>
        <Sec title="Queue" n={(data["infra_queue_dash"] || []).length}>
          <DT rows={data["infra_queue_dash"]} ld={ld}
            ...
            row={(r, i) => <tr key={i} className="border-b border-slate-700/40 hover:bg-slate-700/20"><td className="px-3 py-2 text-xs text-slate-300 max-w-xs truncate">{String(r["queue_name"] ?? "—")}</td><td className="px-3 py-2 text-xs text-slate-300 max-w-xs truncate">{String(r["pending"] ?? "—")}</td><td className="px-3 py-2 text-xs text-slate-300 max-w-xs truncate">{String(r["processing"] ?? "—")}</td><td className="px-3 py-2 text-xs text-slate-300 max-w-xs truncate">{String(r["failed"] ?? "—")}</td><td className="px-3 py-2 text-xs text-slate-300 max-w-xs truncate">{String(r["total"] ?? "—")}</td></tr>}
          />
        </Sec>
        <Sec title="Tools" n={(data["infra_tools_list"] || []).length}>
          <DT rows={data["infra_tools_list"]} ld={ld}
