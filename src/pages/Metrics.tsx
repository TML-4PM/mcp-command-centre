  ...
  const load = useCallback(async () => {
    ...
    try {
      await Promise.allSettled([
        bridgeQueryKey("metrics_portfolio").then(r => setData(p => ({ ...p, "metrics_portfolio": r }))).catch(() => setData(p => ({ ...p, "metrics_portfolio": [] }))),
        bridgeQueryKey("metrics_pos_latest").then(r => setData(p => ({ ...p, "metrics_pos_latest": r }))).catch(() => setData(p => ({ ...p, "metrics_pos_latest": [] }))),
        bridgeQueryKey("metrics_business").then(r => setData(p => ({ ...p, "metrics_business": r }))).catch(() => setData(p => ({ ...p, "metrics_business": [] }))),
      ]);
  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center justify-between flex-wrap gap-3">
        <div><h1 className="text-2xl font-bold">Metrics</h1>
        <p className="text-slate-500 text-xs mt-0.5 font-mono">page_id:metrics · live · no hardcoded data</p></div>
      ...
      <div className="space-y-4">
        <Sec title="Portfolio" n={(data["metrics_portfolio"] || []).length}>
          <DT rows={data["metrics_portfolio"]} ld={ld}
        ...
        <Sec title="POS Latest" n={(data["metrics_pos_latest"] || []).length}>
          <DT rows={data["metrics_pos_latest"]} ld={ld}
            head={() => <tr className="border-b border-slate-700 bg-slate-900/50"><th className="px-3 py-2 text-left text-xs font-semibold text-slate-400 uppercase tracking-wider whitespace-nowrap">business_key</th><th className="px-3 py-2 text-left text-xs font-semibold text-slate-400 uppercase tracking-wider whitespace-nowrap">metric_key</th><th className="px-3 py-2 text-left text-xs font-semibold text-slate-400 uppercase tracking-wider whitespace-nowrap">metric_value</th></tr>}
            row={(r, i) => <tr key={i} className="border-b border-slate-700/40 hover:bg-slate-700/20"><td className="px-3 py-2 text-xs text-slate-300 max-w-xs truncate">{String(r["business_key"] ?? "—")}</td><td className="px-3 py-2 text-xs text-slate-300 max-w-xs truncate">{String(r["metric_key"] ?? "—")}</td><td className="px-3 py-2 text-xs text-slate-300 max-w-xs truncate">{String(r["metric_value"] ?? "—")}</td></tr>}
          />
        </Sec>
        <Sec title="Business Registry" n={(data["metrics_business"] || []).length}>
          <DT rows={data["metrics_business"]} ld={ld}
            head={() => <tr className="border-b border-slate-700 bg-slate-900/50"><th className="px-3 py-2 text-left text-xs font-semibold text-slate-400 uppercase tracking-wider whitespace-nowrap">business_slug</th><th className="px-3 py-2 text-left text-xs font-semibold text-slate-400 uppercase tracking-wider whitespace-nowrap">status</th><th className="px-3 py-2 text-left text-xs font-semibold text-slate-400 uppercase tracking-wider whitespace-nowrap">business_stage</th></tr>}
            row={(r, i) => <tr key={i} className="border-b border-slate-700/40 hover:bg-slate-700/20"><td className="px-3 py-2 text-xs text-slate-300 max-w-xs truncate">{String(r["business_slug"] ?? "—")}</td><td className="px-3 py-2 text-xs text-slate-300 max-w-xs truncate">{String(r["status"] ?? "—")}</td><td className="px-3 py-2 text-xs text-slate-300 max-w-xs truncate">{String(r["business_stage"] ?? "—")}</td></tr>}
          />
        </Sec>
      </div>
    </div>
