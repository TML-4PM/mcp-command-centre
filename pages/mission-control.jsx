...
const INITIAL_ITEMS = [
  ...
  { id:"T008", cat:"MAAT", title:"Import Amex Aug 2024 bank statement", owner:"Troy", priority:"P2", status:"open", notes:"30 coverage gaps" },
  { id:"T009", cat:"MAAT", title:"Import Amex Dec 2024 bank statement", owner:"Troy", priority:"P2", status:"open", notes:"" },
...
const STATUS_CLR = { open:"#e85d3a", done:"#2ecc71", blocked:"#e74c3c", "in-progress":"#f39c12" };
const CAT_CLR = { INFRA:"#4a9eff",BASIQ:"#9b59b6",MAAT:"#1abc9c",RDTI:"#e74c3c",CC:"#f39c12",BRIDGE:"#e85d3a",AGENTS:"#3498db",AGOE:"#2ecc71" };
const P_CLR = { P1:"#e74c3c", P2:"#f39c12", P3:"#555e6e" };
...
export default function MissionControl() {
  ...
  const [newItem, setNewItem] = useState({ cat:"INFRA", title:"", owner:"Troy", priority:"P2", status:"open", notes:"" });
  ...
  const daysLeft = Math.ceil((new Date("2026-04-30") - new Date()) / 86400000);
  ...
  return (
    <div style={{ background:"#070a0d", minHeight:"100vh", fontFamily:"Courier New,monospace", color:"#b8c4d0", fontSize:12 }}>
      <div style={{ background:"#0a0d12", borderBottom:"1px solid #151d28", padding:"12px 18px", display:"flex", alignItems:"center", gap:16, flexWrap:"wrap" }}>
        <div>
          ...
          <div style={{ color:"#2a3a4a", fontSize:9, letterSpacing:2 }}>TECH 4 HUMANITY LTD &middot; ACN 70 666 271 272</div>
        </div>
      ...
      <div style={{ background:"#090c10", borderBottom:"1px solid #111820", padding:"8px 18px", display:"flex", gap:10, flexWrap:"wrap", alignItems:"center" }}>
        <input value={search} onChange={e => setSearch(e.target.value)} placeholder="SEARCH..."
          style={{ background:"#0d1117", border:"1px solid #1e2530", color:"#b8c4d0", padding:"4px 10px", fontFamily:"monospace", fontSize:11, width:150, outline:"none" }} />
        ...
        <button onClick={() => setAddMode(a => !a)}
          style={{ background: addMode?"#0f2010":"#0d1117", border:"1px solid #2ecc7155", color:"#2ecc71", padding:"4px 12px", fontFamily:"monospace", fontSize:11, cursor:"pointer" }}>
          + NEW
        </button>
      </div>
      ...
      {addMode && (
        <div style={{ background:"#0c1219", borderBottom:"1px solid #151d28", padding:"10px 18px", display:"flex", gap:8, flexWrap:"wrap", alignItems:"center" }}>
          ...
          <input value={newItem.title} onChange={e => setNewItem(p=>({...p,title:e.target.value}))}
            ...
            style={{ flex:1, minWidth:200, background:"#0d1117", border:"1px solid #4a9eff55", color:"#b8c4d0", padding:"4px 10px", fontFamily:"monospace", fontSize:11, outline:"none" }} />
          ...
          <input value={newItem.notes} onChange={e => setNewItem(p=>({...p,notes:e.target.value}))}
            ...
            style={{ width:180, background:"#0d1117", border:"1px solid #1e2530", color:"#8899aa", padding:"4px 8px", fontFamily:"monospace", fontSize:11, outline:"none" }} />
          <button onClick={addNew} style={{ background:"#0f2010", border:"1px solid #2ecc71", color:"#2ecc71", padding:"4px 14px", fontFamily:"monospace", fontSize:11, cursor:"pointer" }}>SAVE</button>
          <button onClick={() => setAddMode(false)} style={{ background:"transparent", border:"1px solid #222", color:"#445", padding:"4px 10px", fontFamily:"monospace", fontSize:11, cursor:"pointer" }}>X</button>
        </div>
      )}
            ...
            {filtered.map((item, idx) => {
              ...
              return (
                <tr key={item.id} style={{ borderBottom:"1px solid #0d1117", background:idx%2===0?"transparent":"#080b0f", opacity:done?0.4:1 }}>
                  ...
                  <td style={{ padding:"7px 14px", color:done?"#2a3a4a":"#b8c4d0", maxWidth:320 }}>
                    {editing === item.id
                      ? <input defaultValue={item.title} autoFocus
                          ...
                          style={{ width:"100%", background:"#0d1117", border:"1px solid #4a9eff", color:"#b8c4d0", padding:"2px 6px", fontFamily:"monospace", fontSize:12, outline:"none" }} />
                  ...
                  <td style={{ padding:"7px 14px" }}>
                    <select value={item.status} onChange={e => upd(item.id,"status",e.target.value)}
                      style={{ background:`${STATUS_CLR[item.status]}15`, border:`1px solid ${STATUS_CLR[item.status]}40`, color:STATUS_CLR[item.status], padding:"2px 7px", fontFamily:"monospace", fontSize:10, cursor:"pointer", outline:"none" }}>
                  ...
                  <td style={{ padding:"7px 14px", color:"#3a4a5a", fontSize:10, maxWidth:220 }}>
                    {editing === `${item.id}_n`
                      ? <input defaultValue={item.notes} autoFocus
                          ...
                          onKeyDown={e => e.key==="Enter" && e.target.blur()}
                          style={{ width:"100%", background:"#0d1117", border:"1px solid #4a9eff", color:"#8899aa", padding:"2px 6px", fontFamily:"monospace", fontSize:10, outline:"none" }} />
